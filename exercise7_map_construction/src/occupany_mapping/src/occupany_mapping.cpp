...
void OccupanyMapping(std::vector<GeneralLaserScan> &scans, std::vector<Eigen::Vector3d> &robot_poses)
{
    ...
    for (int i = 0; i < scans.size(); i++)
    {
        ...
        for (int id = 0; id < scan.range_readings.size(); id++)
        {
            ...
            double angle = -scan.angle_readings[id]; // anti-clock
            ...
            // update the grid which is hitted by laser
            if(int(pMap[hitIndex])+mapParams.log_occ > mapParams.log_max)
                pMap[hitIndex] = mapParams.log_max;
            else
                pMap[hitIndex] += mapParams.log_occ;

            std::vector<GridIndex> misses = TraceLine(robotIndex.x, robotIndex.y, hit.x, hit.y);

            // update the grids which are between the robot grid and hitted grid
            for(GridIndex& miss : misses){

                int missIndex = GridIndexToLinearIndex(miss);
                if(int(pMap[missIndex])+mapParams.log_free < mapParams.log_min)
                    pMap[missIndex] = mapParams.log_min;
                else
                    pMap[missIndex] += mapParams.log_free;
            }
...
int main(int argc, char **argv)
{
    ...
    ros::Publisher mapPub = nodeHandler.advertise<nav_msgs::OccupancyGrid>("occupancy_map", 1, true);
